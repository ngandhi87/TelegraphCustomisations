				...
				<fo:flow flow-name="xsl-region-body">
					<xsl:for-each select="XMLData/XMLDataObjects/XMLDataObject">
						...
						<fo:block text-align="center" font-weight="bold" font-size="12pt">
							<xsl:value-of select="/XMLData/XMLDataObjects/XMLDataObject[1]/attributes/attribute[@name='PRINT_Grid_print_of']"/>
						...
						<xsl:choose>
							...
							<xsl:when test="contains('COMPOSITE', primary_key/@type)">
											...
											<fo:table-row>
															...
															<xsl:otherwise>
																<fo:block>
																	<xsl:call-template name="generateimagetag">
																		...
																		<xsl:with-param name="imgURL">
																			<xsl:choose>
																				<xsl:when test="attributes/attribute[@name='has_Preview_Privilege']='true'"><xsl:value-of select="/XMLData/MISCData/attributes/attribute[@name='CONTEXT_URL']"/>/GetAsset?primaryKey=<xsl:value-of select="primary_key"/>&amp;type=p&amp;transformation=iccprofile:mode=stdonly&amp;internal=true&amp;streamAsset=false</xsl:when>
												...
												<fo:table-cell>
													<fo:table table-layout="fixed" font-size="9pt">
														...
														<fo:table-column column-number="3" column-width="66mm" />
								...
								<!-- insert members -->
								<xsl:if test="attributes/attribute[@name='__member_links']/XMLDataObjects/XMLDataObject">
									<xsl:variable name="numMembers" select="count(attributes/attribute[@name='__member_links']/XMLDataObjects/XMLDataObject)" />
									<fo:block>
										<fo:table table-layout="fixed" font-size="10pt">
											<fo:table-column column-number="1" column-width="64mm" />
											<fo:table-column column-number="2" column-width="64mm" />
												...
												<xsl:for-each select="attributes/attribute[@name='__member_links']/XMLDataObjects/XMLDataObject">
													<xsl:if test="position() mod 3 = 1">
														<fo:table-row>
															...
															<xsl:if test="position() + 1 &lt;= $numMembers">
																<fo:table-cell text-align="center" display-align="after">
																	<fo:block>
																		<xsl:apply-templates select="following-sibling::XMLDataObject[1]"/>
																	</fo:block>
																</fo:table-cell>
															</xsl:if>

															<xsl:if test="position() + 2 &lt;= $numMembers">
																<fo:table-cell text-align="center" display-align="after">
																	<fo:block>
																		<xsl:apply-templates select="following-sibling::XMLDataObject[2]"/>
																	</fo:block>
																</fo:table-cell>
															</xsl:if>
														</fo:table-row>
