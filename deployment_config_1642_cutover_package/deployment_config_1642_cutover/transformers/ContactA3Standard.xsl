...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:svg="http://www.w3.org/2000/svg">
	...
	<xsl:template match="*">
		<fo:table table-layout="fixed" font-size="10pt" height="78mm" keep-together.within-page="always">
			<fo:table-column column-number="1" column-width="64mm" />
			<fo:table-body>
				<xsl:choose>
						...
						<fo:table-row height="70mm">
								...
								<fo:block text-align="center">
									<xsl:call-template name="generateimagetag">
										...
										<xsl:with-param name="maxwidth">63</xsl:with-param>
										<xsl:with-param name="maxheight">67</xsl:with-param>
								...
								<xsl:when test="attributes/attribute[@name='PAGE_DISPLAY_TYPE'] = 'version'">
									...
									<fo:block text-align="center" background-color="#FFE79D">
										<xsl:call-template name="generateimagetag">
											...
											<xsl:with-param name="height" select="attributes/attribute[@name='height']"/>
											<xsl:with-param name="maxwidth">63</xsl:with-param>
											<xsl:with-param name="maxheight">69</xsl:with-param>
	...
	<xsl:template match="/">
		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="Liberation Sans">
			...
			<fo:layout-master-set>
				<fo:simple-page-master master-name="PageMaster" page-height="420mm" page-width="297mm" margin-top="7mm" margin-bottom="5mm" margin-left="1cm" margin-right="1cm">
					<fo:region-body margin-bottom="1cm" margin-top="8mm"/>
			...
			<fo:page-sequence initial-page-number="1" master-reference="PageMaster">
				<fo:static-content flow-name="xsl-region-before">
					<fo:block text-align="center" font-size="9pt">
						<xsl:value-of select="/XMLData/XMLDataObjects/XMLDataObject[1]/attributes/attribute[@name='PRINT_Standard_A3_contact']"/> -
				...
				<fo:static-content flow-name="xsl-region-after">
					...
					<fo:table table-layout="fixed" font-size="9pt">
						<fo:table-column column-number="1" column-width="238mm" />
				...
				<fo:flow flow-name="xsl-region-body">
					<fo:table table-layout="fixed" font-size="10pt">
						<fo:table-column column-number="1" column-width="69mm" />
						<fo:table-column column-number="2" column-width="69mm" />
						<fo:table-column column-number="3" column-width="69mm" />
						<fo:table-column column-number="4" column-width="69mm" />
						...
						<fo:table-body>
							...
							<fo:table-row>
								<fo:table-cell>
									<fo:block>
										<xsl:apply-templates select="XMLData/XMLDataObjects/XMLDataObject[position() mod 4 = 1]"/>
									</fo:block>
								</fo:table-cell>

								<fo:table-cell>
									<fo:block>
										<xsl:apply-templates select="XMLData/XMLDataObjects/XMLDataObject[position() mod 4 = 2]"/>
									</fo:block>
								</fo:table-cell>

								<fo:table-cell>
									<fo:block>
										<xsl:apply-templates select="XMLData/XMLDataObjects/XMLDataObject[position() mod 4 = 3]"/>
									</fo:block>
								</fo:table-cell>

								<fo:table-cell>
									<fo:block>
										<xsl:apply-templates select="XMLData/XMLDataObjects/XMLDataObject[position() mod 4 = 0]"/>
									</fo:block>
								</fo:table-cell>
						</fo:table-row>
						</fo:table-body>
